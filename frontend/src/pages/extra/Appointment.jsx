...
function Appointments() {
  ...
  const loadMyAppointments = async () => {
    ...
    if (!token) return;
    try {
      const response = await fetch(`${API_URL}/appointments/patient`, { headers: { Authorization: `Bearer ${token}` } });
      if (!response.ok) throw new Error("Unable to load appointments");
      const data = await response.json();
      setDoctorRequests(data.map((item) => ({
        ...item,
        doctorName: item.doctor_name,
        patientEmail: item.patient_email,
        date: new Date(item.scheduled_for).toLocaleDateString(),
        time: new Date(item.scheduled_for).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
    ...
    const response = await fetch(
      `${API_URL}/appointments`,
      {
        ...
        body: JSON.stringify({
          ...
          appointment_type: bookingForm.type === "online" ? "online" : "in_person",
        }),
      }
    );
  ...
  const cancelAppointment = async (requestId) => {
    if (window.confirm("Are you sure you want to cancel this appointment?")) {
      try {
        const response = await fetch(`${API_URL}/appointments/${requestId}`, { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${localStorage.getItem("token")}` }, body: JSON.stringify({ status: "cancelled" }) });
        if (!response.ok) throw new Error((await response.json()).detail || "Unable to cancel appointment");
  ...
  // `/appointments/patient` is already scoped by the API.  Older records may
  // not include an email, so filtering by that browser-only field made every
  // counter read zero.
  const userAppointments = doctorRequests;
    ...
    >
      ...
      <div className="relative z-10 flex-1 px-4 py-4 sm:px-6 lg:ml-64 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          ...
          <StatCard
            title="Approved"
            value={userAppointments.filter((a) => a.status === "accepted").length}
            icon={<FaCheckCircle />}
            color="sky"
          />
        </div>
                ...
                >
                  <div className="flex flex-wrap justify-between items-start gap-3">
                    ...
                    {appt.status === "accepted" && (
                      appt.type === "online" ? <Link to={`/call/${appt.id}`} className="rounded bg-[#25D366] px-3 py-2 text-sm font-medium text-white">Open WhatsApp</Link> : <span className="text-green-600 text-sm font-medium flex items-center gap-1"><FaCheck/>Confirmed</span>
                    )}
                  </div>
                </div>
              ))}
