...
function Alerts() {
  ...
  const loadAlerts = async () => {
    ...
    try {
      ...
      setAlerts([...notifications, ...riskAlerts].map((item) => ({
        ...item,
        type: item.category || item.severity || "Notification",
        riskLevel: /medium|moderate/i.test(`${item.title || ""} ${item.message || ""}`) || item.severity === "warning" ? "medium" : (["critical", "high", "emergency"].includes(item.severity) || item.category === "risk_alert" ? "high" : "low"),
        icon: ["critical", "high", "emergency"].includes(item.severity) ? "critical" : item.severity === "warning" ? "warning" : "safe",
        status: item.is_read ? "Reviewed" : "Unread",
        time: item.created_at ? new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeStyle: "medium", hour12: true }).format(new Date(item.created_at.endsWith?.("Z") || /[+-]\d\d:\d\d$/.test(item.created_at) ? item.created_at : `${item.created_at}Z`)) : "Just now",
      })));
    } catch (error) { console.error("Notification loading error", error); setAlerts([]); }
  };
    ...
    >
      ...
      <div className="hidden">
        ...
        <div className="mt-8 space-y-2 flex-1 overflow-y-auto">
          ...
          <NavItem label="Alerts" icon={<FaBell />} to="/alerts" active />
      ...
      {/* MAIN CONTENT */}<div className="relative z-10 flex-1 px-4 py-5 pt-20 sm:px-6 lg:ml-64 lg:px-8 lg:pt-6">
        {/* Header */}
