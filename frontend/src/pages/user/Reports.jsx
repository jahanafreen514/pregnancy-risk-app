...
function Reports() {
  ...
  const loadReports = async () => {
    try {
      ...
      setReports(data.map((report) => {
        const risk = (report.title.match(/—\s*(High|Medium|Low)/)?.[1]) || "Low";
        const score = Number(report.summary.match(/Score:\s*(\d+)/)?.[1] || 0);
        const snapshotRisk = report.risk_level || risk;
        return {
          ...report,
          risk: snapshotRisk === "Medium" ? "Moderate" : snapshotRisk,
          score: Number(report.risk_score ?? score),
      ...
      setLoadError(
        status === 401
          ...
          : `Reports could not be loaded${detail ? `: ${detail}` : "."}`
      );
    }
  };
  ...
  const deleteReport = () => alert("Medical reports are retained securely and cannot be deleted from the patient portal.");
  ...
  return (
    ...
    <div className="fixed left-0 top-0 z-40 w-64 bg-white/80 backdrop-blur-2xl border-r border-pink-100/50 p-5 h-screen flex flex-col">
      ...
      {/* MAIN CONTENT */}
      <div className="relative z-10 ml-64 flex-1 px-4 sm:px-6 lg:px-8 py-4 h-screen overflow-y-auto">
        {/* Top Bar */}
        <div className="flex flex-wrap justify-between items-center gap-4 mb-6 sticky top-0 bg-white/30 backdrop-blur-sm py-3 px-4 rounded-2xl -mx-4">
          ...
          <Link to="/profile" aria-label="Open profile" className="flex items-center gap-3 bg-white/80 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm border border-pink-100 hover:shadow-md">
        ...
        {filteredReports.length === 0 ? (
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl p-10 text-center border border-pink-100/50">
            ...
            <p className="text-gray-500 mt-2">
              {loadError || "Complete a pregnancy prediction to generate reports."}
            </p>
          </div>
        ) : (
